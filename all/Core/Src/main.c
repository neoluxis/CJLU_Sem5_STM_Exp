...
E_TASK get_task(void){
	if (HAL_GPIO_ReadPin(KT1_GPIO_Port, KT1_Pin) == 0) {
		return TASK_1;
	}else if (HAL_GPIO_ReadPin(KT2_GPIO_Port, KT2_Pin) == 0) {
		return TASK_3;
	}else if (HAL_GPIO_ReadPin(KT3_GPIO_Port, KT3_Pin) == 0) {
		return TASK_5;
	}
	return TASK_CNT;
}
